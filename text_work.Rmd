...
tf_idf_calc <- narratives_df %>%
      ...
      group_by(`Operating Unit`) %>%
      summarise('Total Words' = sum(total), 'Covid Words' = sum(covid_n), 'Covid TF' = sum(covid_n)/sum(total), 'Num Narratives' = n_distinct(row_num), 'TFIDF' = ifelse(sum(covid_n) == 0, 0, sum(covid_n)*log(n_distinct(row_num)/(sum(covid_n)))/sum(total)))
...
covid_narrative_words <- narrative_words[narrative_words$word %in% icpi_covid_list,]
covid_narrative_words
```

## Histogram of word frequencies

```{r}
ggplot(covid_narrative_words, aes(n/total, fill = `Operating Unit`)) +
...
covid_freq_by_rank <- freq_by_rank[freq_by_rank$word %in% icpi_covid_list,]
covid_freq_by_rank
```
...
```{r}
rank_subset <- covid_freq_by_rank %>%
